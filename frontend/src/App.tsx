  ...
  async function loadDictionaries() {
    try {
      const [machinesResponse, materialsResponse, gasesResponse, defectsResponse] = await Promise.all([
        fetch(`${API_BASE}/dictionaries/machines`),
        fetch(`${API_BASE}/dictionaries/materials`),
        fetch(`${API_BASE}/dictionaries/gases`),
        fetch(`${API_BASE}/dictionaries/defects`),
      ])
    ...
    <main className="page">
      ...
      {activeTab === 'rules' && (
        <section className="card">
          ...
          <p className="muted">Управление правилами рекомендаций из БД.</p>
          <div className="row">
          ...
          {currentSession ? (
            <>
              <section className="card">
                ...
                <p>Станок: {machines.find((item) => item.value === currentSession.machine_name)?.label ?? currentSession.machine_name}</p>
                <p>Материал: {materials.find((item) => item.value === currentSession.material_group)?.label ?? currentSession.material_group}</p>
                <p>Толщина: {currentSession.thickness_mm} мм</p>
                <p>Газ: {gases.find((item) => item.value === currentSession.gas_branch)?.label ?? currentSession.gas_branch}</p>
      ...
      {activeTab === 'operator' && (
        <>
          {!currentSession ? (
            <section className="card">
              <h2>Настройка</h2>
              <p className="muted">Сначала создайте или загрузите сессию</p>
          ...
          <section className="card">
            ...
            <p className="session-meta">
              ...
              {machines.find((item) => item.value === currentSession.machine_name)?.label ?? currentSession.machine_name}
            </p>

            <h3>A) Текущий режим</h3>
            <div className="row">
            ...
            <h3>C) Рекомендация на следующий шаг</h3>
            {!recommendation ? (
            ...
            <h3>D) Результат тестового реза</h3>
