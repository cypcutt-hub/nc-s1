...
def test_upsert_base_modes_creates_missing_machine(sqlite_session_local) -> None:
    ...
    with sqlite_session_local() as db:
        db.add(Material(name="Carbon Steel", material_group="carbon_steel", default_gas_branch="O2"))
        db.commit()
...
def test_upsert_base_modes_updates_existing_base_mode(sqlite_session_local) -> None:
    ...
    with sqlite_session_local() as db:
        material = Material(name="Carbon Steel", material_group="carbon_steel", default_gas_branch="O2")
    ...
    assert modes[0].trust_level == 100
